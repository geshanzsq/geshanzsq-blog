    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-permission="['blog:picture:category:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-permission="['blog:picture:category:update']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-permission="['blog:picture:category:delete']"
          >删除</el-button
        >
      </el-col>
    </el-row>
    ...
    >
      ...
      >
        <template #default="{ row }">
          <router-link :to="'/blog/picture/data/' + row.id" class="link-type">
...
import { page, remove } from '@/api/blog/picture/category'
...
async function handleDelete(row) {
  ...
  proxy.$modal
    .confirm(
      '当前图片分类下可能包含有图片，是否确认删除分类编号为【' +
        deleteIds +
        '】的数据项?'
    )
    .then(async function () {
      await remove(deleteIds)
      proxy.$modal.msgSuccess('删除图片分类成功')
      handleQuery()
    })
}
